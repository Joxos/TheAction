...
def unpack_and_process(package):
    ...
    if package_type == PACKAGE.REQUEST_CHANGE_QUESTION_MARK.name:
        ...
        return pack_answer_login(login(package.get("username"), package.get("password")))
    elif package_type == PACKAGE.REQUEST_REGISTER.name:
        return pack_answer_register(register(package.get("username"), package.get("password")))
    else:
